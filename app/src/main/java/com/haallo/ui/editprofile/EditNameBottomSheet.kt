...
import com.haallo.databinding.BottomSheetEditNameBinding
...
class EditNameBottomSheet(
    ...
    private val saveNameClickSubject: PublishSubject<String> = PublishSubject.create()
    val saveNameClick: Observable<String> = saveNameClickSubject.hide()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.bottom_sheet_edit_name, container, false)
        _binding = BottomSheetEditNameBinding.bind(view)
        return view
    }
    ...
    private fun listenToEvent() {
        ...
        binding.etName.textChanges().subscribeAndObserveOnMainThread {
            binding.tvCount.text = (getString(R.string.max_length_name).toInt() - it.length).toString()
        }.autoDispose()

        binding.btnSave.throttleClicks().subscribeAndObserveOnMainThread {
            if (binding.etName.text.toString().isNotEmpty()) {
                saveNameClickSubject.onNext(binding.etName.text.toString())
            } else {
                showToast(getString(R.string.msg_name_can_not_empty))
            }
        }.autoDispose()
